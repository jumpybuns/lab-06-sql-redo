...
app.get('/shielas', async(req, res) => {
  try {
    const data = await client.query('SELECT shielas.id, shielas.name, categories.category as category from shielas join category on category.id = shielas.category_id order by categories.category desc');
...
app.post('/shielas/', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      INSERT INTO shielas (name, category_id, year, alive, user_id, alias)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, 
